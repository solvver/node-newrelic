...
var DEFAULT_FILENAME = 'newrelic_agents/newrelic';
...
function _findConfigFile() {
  ...
  for (var i = 0; i < CONFIG_FILE_LOCATIONS.length; i++) {
    ...
    var caller_path = require.main.filename.split("/");
    var caller_module=caller_path[caller_path.length-1];
    var def = DEFAULT_FILENAME +"-"+caller_module;

    filepath = path.join(path.resolve(candidate), def);
...
function Config(config) {
  ...
  // If version is 1.11.4 and cat feature flag is on, bump to 1.11.9. The APM UI
  // uses agent version to determine whether to show the CAT map UI. This way we
  // can fully show the feature to customers who turn on the feature flag.
  if ((this.version === '1.11.3' || this.version === '1.11.4') &&
      this.feature_flag.cat) {
    this.version = '1.11.9';
  }
